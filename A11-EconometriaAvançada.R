#Aula 11 - Sazonalidade e Suavização ~ Econometria Avancada
...
install.packages("Hmisc")      #Instalando pacote Hmisc  
install.packages("forecast")   #Instalando pacote forecast
library(Hmisc)                 #Carregando o pacote Hmisc
library(forecast)              #Carregando o pacote forecast
library(readxl)                #Carregando o pacote readxl


IPCA.df<-read_excel("c:/Econometria/IPCA.xls")                       #Sobe o arquivo
plot(IPCA.df$IPCA, type = "l")                                       #Cria um gráfico do arquivo
MM <- data.frame(na.omit(ma(IPCA.df$IPCA,order = 12, centre = T)))   #Cria uma série de médias móveis tradicionais omitindo oa NAS de lag(N)=12    

a <- (127-nrow(MM))+1                                                #Define um parâmetro a para ponderar as perdas de dados para ponderação da média
IPCA.DF <- as.data.frame(IPCA.df$IPCA[a:127])                        #Define IPCA.DF como um vetor do mesmo tamanho que o vetor das médias móveis MM
Tabela1 <- cbind(IPCA.DF,MM)                                         #Cria a Tabela de Dados Tabela1
colnames(Tabela1) <- c("IPCA","Média móvel")                         #Define nomes para as as colunas
View(Tabela1)

Grafico <- ts(Tabela1, start = 2008, frequency = 12)                 #Cria a Série Temporal "Grafico" mensal iniciando em 2008
